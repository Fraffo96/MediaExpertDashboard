/**
 * Market Intelligence – gestione unificata dropdown.
 * Usa MICategoryDropdown e MIGenericDropdown (stile custom come Avg Discount Depth).
 * Popola e collega tutti i controlli (year, channel, category, subcategory, metric, discount, ROI, peak, promo share).
 * Usa: MIDropdowns.populate(d, state, callbacks)
 */
(function() {
  'use strict';
  ...
  var CHANNEL_CONFIG = [
    ...
    { id: 'mi-channel-incremental-yoy', stateKey: 'channel_incremental_yoy', scope: 'incremental_yoy' }
  ];
  ...
  function populate(d, state, callbacks) {
    ...
    if (!state.incr_yoy_cat && !state.incr_yoy_sub && firstCatId) state.incr_yoy_cat = firstCatId;
  ...
  function populateDeepDiveSelects(brandCats, brandSubcats, firstCatId, state, applyView) {
    ...
    setupDeepDive('mi-incremental-yoy-scope-select', { cat: 'incr_yoy_cat', sub: 'incr_yoy_sub' }, 'incremental_yoy');
  }
  ...
  window.MIDropdowns = { populate: populate };
})();
