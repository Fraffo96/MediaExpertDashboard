...
class ExpertAgent:
    ...
    async def _handle_remove_sku(self, *, message: str, brand_id: int, slots: dict) -> tuple[str, dict]:
        ...
        if pid is None:
            ...
            base = (
                "To estimate the impact, please tell me which SKU you mean (preferably the numeric `product_id`, or the exact product name). "
                f"I can also use the default period {ps} to {pe}, unless you specify another range."
            )
            llm = self._llm(
    ...
    async def _handle_add_sku(self, *, message: str, brand_id: int, slots: dict) -> tuple[str, dict]:
        ...
        if not (slots.get("subcategory_id") or slots.get("category_id")):
            base = (
                "Before I can estimate the impact, I need a bit more context:\n"
                "- Which category or subcategory will the new SKU belong to? (e.g., parent category 1–10 or a subcategory id like 301)\n"
                "- What is the expected price positioning vs your current top SKUs? (lower / similar / higher)\n"
                "- Which channel matters most? (web / app / store / all)\n"
                f"If you don’t specify a period, I’ll use {ps} to {pe}."
            )
            llm = self._llm(
                "You are an analytics assistant for Media Expert. Reply in English.\n"
                "Ask 3 short clarifying questions as bullets.\n\n"
                f"User message: {message}\n\n"
                f"Base: {base}"
            )
            return (llm or base, {"slots": slots})
        ...
        llm = self._llm(
            ...
            "DATA (JSON):\n"
            + json.dumps({"period_start": ps, "period_end": pe, "benchmark": bench}, ensure_ascii=False)
            + "\n\nDRAFT:\n"
            + draft
        )
    ...
    def _render_add_sku_answer(self, *, period_start: str, period_end: str, benchmark: list[dict]) -> str:
        lines = [
            ...
            "",
            "To estimate an uplift, I benchmark against your current top SKUs. Here are a few reference points:",
        ]
